...
public class QuestPdfVentasExporter : IVentasPdfExporter
{
    static QuestPdfVentasExporter()
    {
    ...
    public byte[] Export(VentasReporteDto reporte)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Text("Reporte de ventas — Patio El Olvidado")
                ...
                page.Content().PaddingVertical(16).Column(col =>
                {
                    ...
                    col.Item().Text($"Pagos: {reporte.CantidadPagos} | Pedidos: {reporte.CantidadPedidos}");

                    col.Item().PaddingTop(12).Text("Detalle diario").SemiBold().FontSize(12);
                    col.Item().Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            header.Cell().AlignRight().Text("Pagos").SemiBold();
                        });
                        ...
                        foreach (var d in reporte.Dias)
                        {
                            ...
                            table.Cell().AlignRight().Text($"{d.CantidadPagos}");
                        }
                    });
                });
